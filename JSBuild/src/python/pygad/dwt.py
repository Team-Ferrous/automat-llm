...
def haar_1d(arr):
    ...
    for i in range(h):
        output[i] = (arr[2*i] + arr[2*i+1]) / 2
        output[h+i] = (arr[2*i] - arr[2*i+1]) / 2
...
def haar_2d(matrix):
    ...
    # rows
    temp = [haar_1d(row) for row in matrix]

    # columns
    result = [[0]*size for _ in range(size)]

    for col in range(size):
        column = [temp[row][col] for row in range(size)]
        column = haar_1d(column)
...
def inverse_haar_1d(arr):
    ...
    for i in range(h):
        output[2*i]   = arr[i] + arr[h+i]
        output[2*i+1] = arr[i] - arr[h+i]

    return output


def inverse_haar_2d(matrix):
    size = len(matrix)
    ...
    for col in range(size):
        ...
        column = inverse_haar_1d(column)

        for row in range(size):
            temp[row][col] = column[row]

    # rows
    result = [inverse_haar_1d(row) for row in temp]
...
def Encode(array):
    ...
    # simple quantization
    array = [[round(v/10) for v in row] for row in array]
...
def Decode(array):
    ...
    # dequantize
    array = [[v*10 for v in row] for row in array]

    # inverse DWT
    array = inverse_haar_2d(array)

    array = [[Unshift(round(c)) for c in row] for row in array]
